...
lst = passphrases.split('\n')
for i, str in enumerate(lst):
    if len(str.split()) == len(set(str.split())):
        uniq += 1
        valid.append(str.split())
...
for string in valid:
    ordered = list(map(lambda x: ''.join(sorted(x)), string))
    if len(list(filter(lambda x: ordered.count(x) > 1, ordered))) == 0:
        not_an += 1
