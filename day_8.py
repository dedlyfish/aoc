"""commands = b inc 5 if a > 1
a inc 1 if b < 5
c dec -10 if a >= 1
c inc -20 if c == 10"""

commands = """kd dec -37 if gm <= 9
x dec -715 if kjn == 0
ey inc 249 if x < 722
n dec 970 if t > 3
f dec -385 if msg > -3
...
for ptr in commands.split('\n'):
    reg = re.findall(r'^[a-z]+', ptr)[0]
    value = int(re.findall(r'(-?\d+) if', ptr)[0])
    operand = re.findall(r'if (\w+)', ptr)[0]
    if ptr.count('dec'):
        value = -value
    cmp_value = int(re.findall(r'(-?\d+)$', ptr)[0])
    if not registers.get(operand):
        registers[operand] = 0
    if not registers.get(reg):
        registers[reg] = 0
    if ptr.count('=='):
        if registers[operand] == cmp_value:
            registers[reg] += value
    elif ptr.count('!='):
        if registers[operand] != cmp_value:
            registers[reg] += value
...
for ptr in commands.split('\n'):
    ...
    if ptr.count('=='):
        ...
        if registers[operand] > cmp_value:
            registers[reg] += value
    if maximum < registers[reg]:
        maximum = registers[reg]
